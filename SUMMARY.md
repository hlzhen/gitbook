# Summary

* [前言](README.md)
* [环境配置](context-configuration/README.md)
  * [JDK配置](/context-configuration/README.md)
    * [Windows](/context-configuration/java-configuration/windows-jdk-x64.md)
    * [MacOS](/context-configuration/java-configuration/macos-jdk.md)
    * [Linux](/context-configuration/java-configuration/linux-jdk.md)
* [设计模式](design-mode/README.md)
  * [七大原则](design-mode/设计模式-七大原则.md)
  * [创建型模式](design-mode/created/README.md)
    * [工厂模式](design-mode/设计模式-七大原则.md)

* [Git](git-command/README.md)
  * [常用命令](git-command/git常用命令.md)

* [Spring5入门](spring/README.md)
  * [第一章 引言](spring/spring-引言.md)
  * [第二章 注入](spring/spring-注入.md)
    * [1 Set注入详解](spring/spring-set注入.md)
    * [2 构造注入](spring/spring-构造注入.md)
  * [第三章 控制反转 与 依赖注入](spring/spring-控制反转与依赖注入.md)
  * [第四章 Bean的实例控制](spring/spring-scope.md)
  * [第五章 Bean的生命周期](spring/spring-bean-lifecycle.md)
  * [第六章 Bean的后置处理](spring/spring-beanPostProcessor.md)
  * [第七章 Aop编程](spring/aop/README.md)
    * [静态代理](spring/aop/aop-静态代理设计模式.md)
    * [动态代理](spring/aop/aop-动态代理设计模式.md)
    * [AOP编程](spring/aop/aop-编程.md)
    * [基于注解的AOP编程](spring/aop/aop-基于注解编程.md)
    * [AOP总结](spring/aop/aop-总结.md)
  * [第八章 Spring注解](spring/spring-注解详解.md)
  * [第九章 其它](spring/other/README.md)
    * [配置文件参数化](spring/other/spring-配置文件参数化.md)
    * [自定义类型转换](spring/other/spring-自定义类型转换器.md)
    * [Spring整合YML配置文件](spring/other/spring-整合YML配置文件.md)
    * [Spring整合Mybatis](spring/other/web/spring-mybatis.md)
    * [Spring整合Struts2](spring/other/web/spring-struts2.md)
    * [Spring-Struts2-Mybatis(配置化整合)](spring/other/web/spring-struts2-mybatis.md)
